const items = {'紅茶':20, '綠茶': 20, '珍珠奶茶': 35,'咖啡':50 }

const notes = {'去冰':0, '正常冰':0,'微冰':0, '熱':0}

const addsitem = {'無':0,'粉圓':5, '椰果':5, '牛奶':10}

const sugar = {'正常':0,'8分':0, '5分':0, '3分':0,'無':0}
...
const addsitemSelect = document.getElementById('addsitem')
...
function init() {
  sugarSelect.innerHTML = optionList(sugar)
  addsitemSelect.innerHTML = optionList(addsitem)
...
function calcPrice() {
  let [sugar, sugarPrice] = sugarSelect.value.split(':')
  let [addsitem, addsitemPrice] = addsitemSelect.value.split(':')
  let [item, itemPrice] = itemSelect.value.split(':')
  let [note, notePrice] = noteSelect.value.split(':')
  let price = parseInt(itemPrice) + parseInt(notePrice)+parseInt(sugarPrice)+parseInt(addsitemPrice)
  priceControl.value = price
  return {sugar,addsitem,item, note, price}
}

function addItem() {
  let {sugar,addsitem,item, note, price} = calcPrice()
  let quantity = parseInt(quantityControl.value)
  let record = {name: item+'('+note+')'+'('+sugar+')'+'('+addsitem+')', price: price, quantity: quantity}
