...
<thead>
  <tr>
    <td>
      ...
      <select id="addsitem" onchange="Pos.calcPrice()"></select>
      <select id="sugar" onchange="Pos.calcPrice()"></select>
    </td>
...
Pos.start = function () {
  ...
  Ui.id('addsitem').innerHTML = Pos.optionList(Shop.addsitem)
  Ui.id('sugar').innerHTML = Pos.optionList(Shop.sugar)
...
Pos.calcPrice = function () {
  ...
  let [addsitem, addsitemPrice] = Ui.id('addsitem').value.split(':')
  let [sugar, sugarPrice] = Ui.id('sugar').value.split(':')


  let price = parseInt(itemPrice) + parseInt(addonPrice)+ parseInt(addsitemPrice)+ parseInt(sugarPrice)


  Ui.id('price').value = price
  return {sugar,addsitem,item, addon, price}
}

Pos.addItem = function () {
  let {addsitem,sugar,item, addon, price} = Pos.calcPrice()
  let quantity = parseInt(Ui.id('quantity').value)
  let record = {name: item+'('+addon+')'+'('+addsitem+')'+'('+sugar+')', price: price, quantity: quantity}
